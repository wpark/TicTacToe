...
public class Game {
    ...
    public String checkGameWinner(char[][] grid){
        ...
        // Check the horizontal axis and then the vertical axis
        for (int i=0; i<3; i++) {
            for (int j=0; j<3; j++) {
                if ( (gridAt(i,j) != '-') && (gridAt(i,j) != '!') ) {
                    if ( ( (gridAt(i,j) == gridAt(i+1,j)) && (gridAt(i+1,j) == gridAt(i+2,j)) )
                        || ( (gridAt(i,j) == gridAt(i,j+1)) && (gridAt(i,j+1) == gridAt(i, j+2)) ) ) {
                        if (gridAt(i,j) == 'o') {
        ...
        if ( (gridAt(1,1) != '-') && (gridAt(1,1) != '!') ) {
            if ( ((gridAt(0,0) == gridAt(1,1))&&(gridAt(1,1) == gridAt(2,2)) )
                    || ((gridAt(2,0)==gridAt(1,1))&&(gridAt(1,1)==gridAt(0,2))) ) {
                if(gridAt(1,1) == 'o') {
                    return result = "O wins";
                }
                else {
        ...
        for (int i=0; i<3; i++) {
            for (int j=0; j<3; j++) {
                if ( gridAt(i,j) == '-') { // There's a cell that hasn't been filled!
                    return result = "None";
                }
            }
        }
